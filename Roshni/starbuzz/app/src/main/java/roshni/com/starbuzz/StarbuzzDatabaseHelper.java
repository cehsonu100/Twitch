...
class StarbuzzDatabaseHelper extends SQLiteOpenHelper{
    ...
    private void updateMyDatabase(SQLiteDatabase db, int oldVersion, int newVersion) {
        if (oldVersion < 1) {
            ...
            insertDrink(db, "Birthday", "Assist to wish birthday to someone", R.drawable.birthday);
            insertDrink(db, "Meeting", "Attend meeting on time", R.drawable.meeting);
            insertDrink(db, "Anniversary", "To wish birthday", R.drawable.anniversary);
        }
        if (oldVersion < 2) {
